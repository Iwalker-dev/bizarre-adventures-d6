...
export function canUserExecuteAction(messageId, actionType, quadrantNum, actionArg = "") {
    ...
    if (isMessageLocked(message)) {
        ui.notifications.error("Message is Locked");
        return false;
    };
    if (isMessageResolved(message) && !isPostResolveAllowedAction(actionType, actionArg)) {
        ui.notifications.error("You can't make any futher changes to this result.");
        return false;
    };
    ...
    if (actionType === "unready" || actionType === "luck") {
        ...
        if (!Number.isInteger(quadrantNumber)) {
            ui.notifications.error("Invalid Quadrant");
            return false;
        };
        const doesOwn = getQuadrantOwnerState(message, quadrantNumber).isOwner;
        if (!doesOwn) {
